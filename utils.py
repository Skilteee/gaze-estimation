...
from model import RegressionMLP,RegressionRNN
...
def project2screen(width_pixels, height_pixels, feature_queue):

    model = RegressionMLP()
    weight = torch.load('regression_model.pth')
    model.load_state_dict(weight)

    if type(model) == RegressionRNN:
        input = torch.stack(feature_queue[:-1] + [feature_queue[-1] for _ in range(feature_queue.maxsize - len(feature_queue) + 1)], dim=1)
    else:
        input = feature_queue[-1]
    ...
    # 将result中的值限制在屏幕内并转换成int
    result = np.clip(result, 0, [width_pixels, height_pixels])
...
def hybrid_calibration(cam_eye_point, target_screen_point, gaze_pitch, gaze_yaw):
    """
    ...
    def objective_function(variables):
        ...
        screen_eye_point = np.dot(rotation_mat, cam_eye_point.T).T + translation_mat

        x_screen = screen_eye_point[:, -1] * np.tan(gaze_yaw) + screen_eye_point[:, 0]
        y_screen = screen_eye_point[:, -1] * np.tan(gaze_pitch) + screen_eye_point[:, 1]
