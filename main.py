...
from utils import get_camera_matrix, get_face_landmarks_in_ccs, gaze_2d_to_3d, ray_plane_intersection, plane_equation, get_monitor_dimensions, get_point_on_screen, gaze_3dto2d, project2screen, Queue
...
def main(calibration_matrix_path: str, monitor_mm=None, monitor_pixels=None, model=None, visualize_preprocessing=False, visualize_laser_pointer=True, visualize_3d=False):
    # setup webcam
    source = WebcamSource(width=640, height=480, fps=60, buffer_size=10)
    ...
    for frame_idx, frame in enumerate(source):
        ...
        if results.multi_face_landmarks:
            ...
            pitch, yaw = gaze_3dto2d(gaze_vector)

            input_vec = torch.Tensor(
                    np.concatenate([(left_eye_center + right_eye_center) / 2, np.array(pitch).reshape(-1, 1), np.array(yaw).reshape(-1, 1)], axis=0)).T

            feature_queue.append(input_vec)

            point_on_screen = tuple(project2screen(monitor_pixels[0], monitor_pixels[1], feature_queue).tolist()[0])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_path", type=str, default='./pretrained_gaze_model.ckpt')
